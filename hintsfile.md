...
We start by determining $\ell$ for a given $n$ and $m$. For each $i$, take the $\ell$ least significant bits and append them to a bitset $L$. Next, iterate over each element and take the remaining most significant bits, then record the difference between the last element's high bits and the current element's high bits. Record that number using the $\text unary$ function, and append the unary to a vector $H$. The final representation is $m$, $n$, $L$, $H$.
## Worked Example
